...
things_dir = '/raid/shared/datasets/visoin/things/'
project_dir = f'{things_dir}/MEG1/'
...
fif_file = os.path.join(project_dir, f"/derivatives/preprocessed/preprocessed_P{sub}-epo.fif")
...
concept_csv_file_path = os.path.join(things_dir,
                                     'THINGS/Metadata/Concept-specific/image_concept_index.csv')
csv_img_file_path = os.path.join(things_dir, "THINGS/Metadata/Image-specific/image_paths.csv")

origin_img_dir = os.path.join(things_dir, "THINGS/Images/")
...
for i, event_id in enumerate(zs_event_ids):
    ...
    image_category_index = image_concept_df.iloc[
        event_id - 1, 0]  # Accessing the first (and only) column at row i
...
train_set_categories_filtered = [item for item in train_set_categories if
                                 item not in test_set_categories]
...
def reshape_meg_data(epochs, num_concepts, num_imgs, repetitions):
    ...
    reshaped_data = data.reshape(
        (num_concepts * num_imgs, repetitions, data.shape[1], data.shape[2]))
    return reshaped_data


training_data = reshape_meg_data(training_epochs_filtered, num_concepts=1654, num_imgs=12,
                                 repetitions=1)
...
img_path_list_training = [path.split('data/things-meg/Image_set/')[1] for path in
                          img_path_list_training]
