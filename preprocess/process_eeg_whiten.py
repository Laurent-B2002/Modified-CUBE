...
project_dir = './things/EEG2/'

if whiten:
    save_dir = os.path.join(
        project_dir, f'Preprocessed_data_{re_sfreq}Hz_whiten', 'sub-' + format(sub, '02')
    )
else:
    save_dir = os.path.join(
        project_dir, f'Preprocessed_data_{re_sfreq}Hz_no_whiten', 'sub-' + format(sub, '02')
    )
...
def mvnn(epoched_test, epoched_train):
    ...
    for s in range(n_ses):
        ...
        for p in range(sigma_part.shape[0]):
            ...
            sigma_cond = np.empty(
                (session_data[p].shape[0], session_data[0].shape[2], session_data[0].shape[2])
            )
            for i in tqdm(range(session_data[p].shape[0])):
                ...
                if mvnn_dim == "time":
                    sigma_cond[i] = np.mean(
                        [_cov(cond_data[:, :, t], shrinkage='auto') for t in
                         range(cond_data.shape[2])],
                        axis=0
                    )
                ...
                # and then average across epochs/repetitions
                    ...
                    sigma_cond[i] = np.mean(
                        [_cov(np.transpose(cond_data[e]), shrinkage='auto') for e in
                         range(cond_data.shape[0])],
                        axis=0
                    )
        ...
        whitened_test.append(
            np.reshape(
                ...
                ).swapaxes(1, 2) @ sigma_inv).swapaxes(1, 2), session_data[0].shape
            )
        )
        whitened_train.append(
            np.reshape(
                (np.reshape(
                    session_data[1], (-1, session_data[1].shape[2], session_data[1].shape[3])
                ).swapaxes(1, 2) @ sigma_inv).swapaxes(1, 2), session_data[1].shape
            )
        )
...
def epoch_data(mode, sub):
    ...
    for s in range(n_ses):
        ...
        eeg_dir = os.path.join(
            'Raw_data', 'sub-' + format(sub, '02'), 'ses-' + format(s + 1, '02'),
            f"raw_eeg_{mode}.npy"
        )
        ...
        max_rep = 20 if mode == 'test' else 2
...
img_directory = f'data/things-eeg/test_images/'
images = []
labels = []
texts = []
all_images = sorted([d for d in os.listdir(img_directory)])
...
img_directory = f'data/things-eeg/train_images/'
images = []
labels = []
texts = []
all_images = sorted([d for d in os.listdir(img_directory)])
