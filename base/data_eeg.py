...
def load_eeg_data(config):
    ...
    if exp_setting == 'intra-subject':
        ...
        test_loader = DataLoader(test_dataset, batch_size=config['data']['test_batch_size'], shuffle=False, drop_last=False,num_workers=0, pin_memory=True)
        train_loader = DataLoader(train_dataset, batch_size=config['data']['train_batch_size'], shuffle=True, drop_last=False, num_workers=0, pin_memory=True)
        ...
        print('init train_dataset success')
        test_loader = DataLoader(test_dataset, batch_size=config['data']['test_batch_size'], shuffle=False, drop_last=False,num_workers=0)#, pin_memory=True)
        val_loader = DataLoader(val_dataset, batch_size=config['data']['val_batch_size'], shuffle=False, drop_last=False,num_workers=0)#, pin_memory=True)
        train_loader = DataLoader(train_dataset, batch_size=config['data']['train_batch_size'], shuffle=True, drop_last=False, num_workers=0)#, pin_memory=True)
...
class EEGDataset(Dataset):
    def __init__(self, config, mode):
        ...
        self.leison_time = config['data']['leison_time']
        ...
        features_filename = f"{config['db_dir']}/{mode}.pt"
        if mode == 'train':
            self.colour_hists = np.load(f"{config['colour_gt_path']}", allow_pickle=True)[()]
        else:
            directory = os.path.dirname(os.path.abspath(config['colour_gt_path']))
            self.colour_hists = np.load(f"{directory}/test.npy", allow_pickle=True)[()]
...
###
        print(
            "EEG stats:",
            loaded_data['eeg'].min().item(),
            loaded_data['eeg'].max().item(),
            loaded_data['eeg'].mean().item(),
            loaded_data['eeg'].std().item()
        )
        
        print("BEFORE AVG:", loaded_data['eeg'].shape)
###
        if self.selected_ch:
        ...
        # if self.mode == "train":
        #     loaded_data['eeg'] = loaded_data['eeg'][:200]
        #     loaded_data['label'] = loaded_data['label'][:200]
        #     loaded_data['img'] = loaded_data['img'][:200]
        #     loaded_data['text'] = loaded_data['text'][:200]
        #     loaded_data['session'] = loaded_data['session'][:200]

        # elif self.mode == "test":
        #     loaded_data['eeg'] = loaded_data['eeg'][:50]
        #     loaded_data['label'] = loaded_data['label'][:50]
        #     loaded_data['img'] = loaded_data['img'][:50]
        #     loaded_data['text'] = loaded_data['text'][:50]
        #     loaded_data['session'] = loaded_data['session'][:50]
    ...
    def __getitem__(self, index):
        ...
        if self.avg:
            eeg_mean = eeg
            #for testing
            # eeg_mean = (eeg_mean - eeg_mean.mean()) / (eeg_mean.std() + 1e-6)
        else:
        ...
        # colour features
        colour_gt = torch.tensor(self.colour_hists[img_path], dtype=torch.float)
        ...
        if self.leison_time is not None:
            eeg[:, self.leison_time] = 0
            eeg_mean[:, self.leison_time] = 0
